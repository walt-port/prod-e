...
# Script to fix Prometheus configuration
# This script updates the Prometheus task definition to fix routing issues
...
echo -e "${YELLOW}Fixing Prometheus configuration...${NC}"
...
jq '.taskDefinition.containerDefinitions[0].command = ["--config.file=/etc/prometheus/prometheus.yml", "--storage.tsdb.path=/prometheus", "--web.console.libraries=/usr/share/prometheus/console_libraries", "--web.console.templates=/usr/share/prometheus/consoles"]' prom-task-def.json > prom-task-def-updated.json
...
echo -e "${GREEN}Prometheus configuration update completed. The service will deploy a new task with the updated configuration.${NC}"
